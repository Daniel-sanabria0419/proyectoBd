...
try:
    client = MongoClient("mongodb+srv://root:12345@clustertienda.xy11p.mongodb.net/?retryWrites=true&w=majority&tls=true&tlsAllowInvalidCertificates=true")
    mongo_db = client["clustertienda"]
...
def api_obtener_compras():
    try:
        compra = obtener_compras()  # Llamamos a la función que obtiene las compras
...
def api_insertar_compra():
    try:
        ...
        insertar_compra(cliente_id, producto_id, sucursal_id,cantidad)
...
def api_insertar_cliente():
    try:
        ...
        insertar_cliente(nombre, correo, latitud, longitud)
        return jsonify({"message": "Registrado exitosamente"}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
