...
def insertar_compra(cliente_id, producto_id, sucursal_id,cantidad):
    try:
        # Inserta los datos en la tabla Compras
        fecha = datetime.now()  # Genera 'YYYY-MM-DD HH:MM:SS'
        query = """
        INSERT INTO compras (cliente_id, producto_id, sucursal_id,cantidad,fecha)
        VALUES (%s, %s, %s,%s,%s)
        """
        values = (cliente_id, producto_id, sucursal_id,cantidad,fecha)
...
def insertar_cliente(nombre,correo,latitud,longitud):
    try:
        # Inserta los datos en la tabla Compras
        query = """
        ...
        VALUES (%s, %s, %s, %s)
        """
        values = (nombre, correo, latitud, longitud)
        cursor.execute(query, values)
        mysql_conn.commit()
        print("Cliente registrada exitosamente.")
    except Exception as e:
        print(f"Error al registrar el cliente: {query}")
